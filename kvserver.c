...
FILE *trace_fp; // trace file
pthread_mutex_t trm = PTHREAD_MUTEX_INITIALIZER; // mutex
...
{
    if (trace_fp != NULL) {
        fclose(trace_fp); // close trace_fp if open
    }
...
void init(void) {
    if (signal(SIGINT, cleanup) == SIG_ERR) {
        perror("catch SIGINT:"); //if signal results in error print signal
    }
    for (int i = 0; i < TABLE_MAX; i++) {
        kv.keys[i].stat = 0; // set key values to FREE
    }
    if (debug) {
        trace_fp = fopen("./kvserver.trace", "w"); //open kserver.trace with write permissions if debug is set to 1
    }
}


void _trace(char *msg) {
    pthread_mutex_lock(&trm); // lock pthread_mutex trm
    fprintf(trace_fp, "%s\n", msg); // write message to trace_fp
    pthread_mutex_unlock(&trm); // unlock pthread_mutex trm
}

void trace(char *msg) {
    if (!debug) {return;} // if not debugging return
    _trace(msg); // else run _trace with message
}

// === dispatcher function ===

req_t *parsereq(char *line) {
    req_t *ret = malloc(sizeof(req_t)); //allocate space for req_t [ret]
    assert(line[1] == ','); // ensures the second character of line is ','
    ret->op = line[0]; //dereference ret -> op and sets op to the first character of line

    switch(ret->op) { //switch statement on op
        case 'R': // fall through
        case 'D': // fall through
...
void *dispatcher(void *file) {
    ...
    struct stat sb;
    if (fstat(fd, &sb) == -1) { // stores information about file into sb (prints error if file did not open properly)
        perror("[Error] getting file size");
        return NULL;
    }

    char *mapped = mmap(NULL, sb.st_size, PROT_READ, MAP_PRIVATE, fd, 0); // map file to memory
    if (mapped == MAP_FAILED) {
        perror("[Error] mmapping file");
        return NULL;
    }
    char *buf = malloc(sb.st_size); //create buffer array of file size
    memcpy(buf, mapped, sb.st_size); // copy mapped file into buffer

    // parse and enqueue the inputs
    int op_counter = 0; //counter for num of operations
    char *ptr; 
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < num_thread; i++){
    	...
    	pthread_create(&workerThread, NULL, worker, (void*) &args );
    }
    ...
    for (;;) {
        ...
        if (strcmp(cmd, "quit") == 0) {
            ...
            // Create disatcher thread
            
           
            pthread_t dispatcherThread;
            pthread_create(&dispatcherThread, NULL, dispatcher, (void*) &file);
